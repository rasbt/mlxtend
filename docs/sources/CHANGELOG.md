...
- `onehot transform` is now more memory efficient as it uses boolean instead of ints. Furthermore, `onehot transform` can be now called with `sparse` argument resulting in the sparse representation of the `onehot` matrix. ([#328](https://github.com/rasbt/mlxtend/pull/328) by [Jakub Smid](https://github.com/jaksmid))
