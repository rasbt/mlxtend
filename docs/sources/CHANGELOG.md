...
- utils.Counter now accepts a name variable to help distinguish between multiple counters, time precision can be set with the 'precision' kwarg, and time since initialization and iteration start cab be accessed by the total_elapsed() and iteration_elapsed() methods. [#278](https://github.com/rasbt/mlxtend/pull/278)
