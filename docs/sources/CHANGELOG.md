...
- The `n_folds` parameter of the `StackingCVRegressor` was changed to `cv` and can now accept any kind of cross validation technique for regression that is available from scikit-learn. For example, `StackingCVRegressor(..., cv=KFold(n_splits=3)) or StackingCVClassifier(..., cv=GroupKFold(n_splits=3))`.
